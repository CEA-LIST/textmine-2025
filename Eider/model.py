...
class DocREModel(nn.Module):
    def __init__(self, config, model, ablation='atlop', max_sent_num=25, emb_size=768, block_size=64, num_labels=-1, att_size=128, b_hid_size=194):
        super().__init__()
        self.config = config
        self.model = model
    ...
    def get_hrt(self, sequence_output, attention, entity_pos, hts, sent_pos=None):
        ...
        for i in range(len(entity_pos)): # for i in batch_size
            ...
            if sent_pos is not None:
                # logsumexp:
                ss = [torch.logsumexp(sequence_output[i, s[0]+offset:s[1]+offset, :], dim=0).unsqueeze(0) for s in sent_pos[i] if s[0]+offset < c]
                ss = torch.cat(ss, dim=0).to(sequence_output.device) # [sen_num, emb_size]
                # padding ss to [max_sent_num, emb_size]
                pad = nn.ZeroPad2d((0, 0, 0, self.max_sent_num - ss.shape[0]))
                ss = pad(ss)

                p_num = len(hts[i])
                sss.append(ss.unsqueeze(0).expand(p_num, self.max_sent_num, self.hidden_size))
    ...
    def forward(self,
        ...
        if labels is not None:
            ...
            if sent_labels is not None:  # 'eider'
                max_sent_num = self.max_sent_num 

                # Initialisation de s_labels avec des zéros avec la forme [len(sent_labels), max_sent_num]
                s_labels = torch.zeros((len(sent_labels), max_sent_num)).to(ss)

                # Remplir s_labels avec les valeurs de sent_labels
                for i, s_label in enumerate(sent_labels):
                    s_label = s_label.to(ss)
                    # Tronquer ou compléter s_label_tensor pour qu'il ait max_sent_num éléments
                    if s_label.shape[0] > max_sent_num:
                        # Tronquer si plus de max_sent_num éléments
                        s_label = s_label[:max_sent_num]
                    elif s_label.shape[0] < max_sent_num:
                        # Compléter avec des zéros si moins de max_sent_num éléments
                        padding = torch.zeros(max_sent_num - s_label.shape[0]).to(ss)
                        s_label = torch.cat((s_label, padding), dim=0)

                    # Assigner à s_labels à l'index correspondant
                    s_labels[i] = s_label
                ...
                s1 = [ss_simple[i].unsqueeze(0).expand(num_idx_used[i], max_sent_num, self.hidden_size) for i in range(ss_simple.shape[0])]
                s1 = torch.cat(s1, dim=0)  # (bs', sents_num, hid)
                s1 = s1.view(-1, max_sent_num, self.hidden_size // self.block_size, self.block_size)  # (bs', sents_num, #bl, bl_size)
                r2 = rs.view(-1, self.hidden_size // self.block_size, self.block_size)  # (bs', #bl, bl_size)
                bl_sr = (s1.unsqueeze(4) * r2.unsqueeze(1).unsqueeze(3)).view(-1, max_sent_num, self.hidden_size * self.block_size)
                # s1 -> (bs', sents_num, #bl, bl_size, 1); r2 -> (bs', 1, #bl, 1, bl_size)
                s_logits = self.sr_bilinear(bl_sr)  # [bs, sents_num, num_labels]

                s_logits = torch.max(s_logits, dim=-1)[0].view(-1, max_sent_num)  # choose the highest prob
                evi_loss = F.binary_cross_entropy_with_logits(s_logits.float(), s_labels.float()) 
                loss = loss + 0.2 * evi_loss
                ...
                if return_senatt:
                    s_output = self.loss_fnt.get_label(s_logits, num_labels=self.num_labels).view(-1, self.max_sent_num, self.num_rels)
            ...
            s1 = ss.view(-1, self.max_sent_num, self.hidden_size // self.block_size, self.block_size) # (bs', sents_num, #bl, bl_size)
            r2 = rs.view(-1, self.hidden_size // self.block_size, self.block_size) # (bs', #bl, bl_size)
            bl_sr = (s1.unsqueeze(4) * r2.unsqueeze(1).unsqueeze(3)).view(-1, self.max_sent_num, self.hidden_size * self.block_size)
            # s1 -> (bs', sents_num, #bl, bl_size, 1); r2 -> (bs', 1, #bl, 1, bl_size)
            s_logits = self.sr_bilinear(bl_sr) # [bs, sents_num, num_labels]

            s_logits = torch.max(s_logits, dim=-1)[0].view(-1, max_sent_num) # choose the highest prob, [bs', sents_num]
            s_output = s_logits > 0
            output = output + (s_output, )
